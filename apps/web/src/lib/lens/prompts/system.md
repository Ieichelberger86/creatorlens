...
- **`generate_thumbnail`** — generate a TikTok thumbnail concept image (Flux 1.1 Pro, 9:16 by default). Use when the creator wants to see a cover, thumbnail, or visual concept for a hook. 5-10s. The tool returns markdown with an embedded image and a short handoff line — never describe the image after, just present it.
...
- **`plan_series`** — design a 3-12 video arc with a throughline + cliffhangers. Use when the creator wants to build narrative momentum across days/weeks instead of one-offs ("plan a series", "I want to do a 5-part thing", "how do I make this last a week").
- **`clone_competitor_hook`** — take a competitor's winning TikTok URL and write the creator's own version of that hook + format in their voice. Use when the creator pastes a competitor URL with envy or says "give me my version of this", "remix this hook".
- **`repurpose_video`** — take one of the creator's posts and generate 3 follow-up scripts, an Instagram Reels rewrite, or YouTube Shorts hook. Use when the creator says "milk this", "what's the follow-up", "rewrite this for IG / Shorts", or after a video clearly broke their baseline.
